...
class AppThemeModeNotifier extends StateNotifier<ThemeMode> {//ThemeMode chính là kiểu dữ liệu của state.
  ...
  void toggleTheme() {
    ...
    //Khi gán giá trị mới cho state, Riverpod sẽ notify tất cả widget đang watch provider → UI đổi theme ngay lập tức.
  }
  ...
  void getCurrentTheme() async {
    ...
    state = value;
    // cập nhật state (lúc đầu light, sau khi đọc xong sẽ set lại).
  }
}

class AppTheme {
  //static → thuộc về class chứ không thuộc về instance.
  //
  // Nghĩa là bạn không cần tạo object AppTheme mới để gọi:
  //final theme = AppTheme.darkTheme; // dùng trực tiếp
  //Nếu không static, bạn sẽ phải:
  // final themeObj = AppTheme();
  // final theme = themeObj.darkTheme;
